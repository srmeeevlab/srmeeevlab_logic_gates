...
!function($s) {
  ...
  var createLogicGateFactory = function(op, out, draw, gateClass) {
    return function(device) {
        var numInputs = (op == null) ? 1 : Math.max(2, device.deviceDef.numInputs || 2);
    ...
    return function(device) {
      ...
      device.createUI = function() {
        ...
        var update = function() {
          ...
          // Check if connected gate is valid and has an output
        ...
        device.doc = {
          params: [
            {name: 'color', type: 'string', defaultValue: defaultLEDColor, description: 'color in hexadecimal.'},
            {name: 'bgColor', type: 'string', defaultValue: defaultLEDBgColor, description: 'background color in hexadecimal.'}
          ],
          code: '{"type":"' + device.deviceDef.type + '","color":"' + defaultLEDColor + '"}'
        };
      };
  ...
  $s.registerDevice('Breadboard', function(device) {
    var SIZE_UNIT = 5;
    var rows = device.deviceDef.rows || 64;
    var cols = device.deviceDef.cols || 10;
    var width = cols * SIZE_UNIT;
    var height = rows * SIZE_UNIT;
    var inPins = [];
    var outPins = [];
    var pins = [];
    var mouseDownPos = null;
    var mouseDownP = null;
    var connectionCheck = function(p1, p2) {
      if (p1.getDevice() == p2.getDevice()) {
        return false;
      }
      var a1 = p1.getAnchors();
      var a2 = p2.getAnchors();
      for (var i = 0; i < a1.length; i += 1) {
        for (var j = 0; j < a2.length; j += 1) {
          var dx = a1[i].getX() - a2[j].getX();
          var dy = a1[i].getY() - a2[j].getY();
    ...
    for (var i = 0; i < pins.length; i += 1) {
      var p1 = pins[i];
      for (var j = i + 1; j < pins.length; j += 1) {
        var p2 = pins[j];
        if (connectionCheck(p1, p2)) {
          $s.createWire(p1, p2);
        }
      }
    }
    device.addInputPins(inPins);
    device.addOutputPins(outPins);
    var super_createUI = device.createUI;
    device.createUI = function() {
      super_createUI();
      device.$ui.addClass('simcir-basicset-breadboard');
      device.$ui.css('width', width + 'px');
      device.$ui.css('height', height + 'px');
      var $board = $s.createSVGElement('rect').
        attr('x', 0).attr('y', 0).
        attr('width', width).attr('height', height).
        attr('fill', 'white').attr('stroke', 'black');
      device.$ui.append($board);
      for (var i = 0; i < pins.length; i += 1) {
        device.$ui.append(pins[i].createUI());
      }
      device.$ui.on('mousedown touchstart', function(event) {
        mouseDownPos = $s.mousePosition(event);
        mouseDownP = null;
        for (var i = 0; i < pins.length; i += 1) {
          var p = pins[i];
          if ($s.hitTest(p.$ui, mouseDownPos)) {
            mouseDownP = p;
            break;
          }
        }
        if (mouseDownP !=  null) {
          ...
          device.$ui.on('mousemove touchmove', function(event) {
          if (mouseDownP !== null) {
          event.preventDefault();
          var pos = $s.mousePosition(event);
          mouseDownP.setPosition(pos.x, pos.y);
          $s.redrawWires();
          }
          });
          device.$ui.on('mouseup touchend', function(event) {
          if (mouseDownP !== null) {
          event.preventDefault();
          var pos = $s.mousePosition(event);
          var hitTest = false;
          for (var i = 0; i < pins.length; i += 1) {
          var p = pins[i];
          if (p !== mouseDownP && $s.hitTest(p.$ui, pos)) {
          hitTest = true;
          if (connectionCheck(mouseDownP, p)) {
          $s.createWire(mouseDownP, p);
          }
          break;
          }
          }
          if (!hitTest) {
          mouseDownP.setPosition(mouseDownPos.x, mouseDownPos.y);
          }
          $s.redrawWires();
          }
          mouseDownPos = null;
